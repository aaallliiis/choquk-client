...
import { OrientationColumns } from "../../../assets/columns";
...
export default function Orientations() {
  ...
  const handleAdd = () => {
    if (!clickedItem.name || !clickedItem.fieldId)
      handleOpenSnack(errorsMessages.emptyField, snackbarTypes.error);
    else {
      if (clickedItem.id) {
        editOrientation({ id: clickedItem.id, body: clickedItem })
  ...
  const handleDelete = () =>
    deleteOrientation(clickedItem.id)
      .then((res) => handleOpenSnack(res, snackbarTypes.success))
      .then(GetAllAdminCourses)
